...
from timm.models.layers import DropPath, to_2tuple, to_ntuple, trunc_normal_, _assert
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        """
        B_, N, C = x.shape # B_: num_win*B, N: window_size*window_size, C: C
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4) # 3, B_, num_head, N, head_dim
        q, k, v = qkv.unbind(0)  # make torchscript happy (cannot use tensor as tuple). B_, num_head, N, head_dim

        q = q * self.scale
        attn = (q @ k.transpose(-2, -1)) # B_, num_head, N, N (broad casting)
        ...
        if mask is not None:
            ...
            attn = self.softmax(attn) # B_, num_head, N, N

        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B_, N, -1) # B_, num_head, N, head_dim => B_, N, C
        x = self.proj(x) # B_, N, C
        x = self.proj_drop(x)
        return x # B_ (=num_win*B), N (=window_size*window_size), C
...
class SwinTransformer(nn.Module):
    ...
    def init_weights(self, mode=''):
        ...
        get_init_weights_vit(mode, head_bias=head_bias)
...
def _load_pretrained(model: SwinTransformer, **kwargs):
...
def swin_tiny_patch4_window7_224(pretrained=False, **kwargs):
    ...
    model.default_cfg = default_cfgs['swin_tiny_patch4_window7_224']
    if pretrained:
...
def swin_small_patch4_window7_224(pretrained=False, **kwargs):
    ...
    model.default_cfg = default_cfgs['swin_small_patch4_window7_224']
    if pretrained:
        model = _load_pretrained(model, **kwargs)
    return model
...
def swin_base_patch4_window7_224(pretrained=False, **kwargs):
    ...
    model.default_cfg = default_cfgs['swin_base_patch4_window7_224']
    if pretrained:
        model = _load_pretrained(model, **kwargs)
    return model
...
def swin_base_patch4_window7_224_in22k(pretrained=False, **kwargs):
    ...
    model.default_cfg = default_cfgs['swin_base_patch4_window7_224_in22k']
    if pretrained:
        model = _load_pretrained(model, **kwargs)
    return model